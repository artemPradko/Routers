...
//     calculateAllUserBalance(json)

//     showAllUniqueTags(json)

//     getAllUserDescription(json)
...
function test(entries) {
    ...
    filterWithCardType.forEach(el => {
        console.info(`This user ${el.first_name} ${el.last_name} have card ${el.credit_card_type}, number of card ${el.credit_card_number}`)
    })
...
function calculateAllMockDataMoney(entries) {
    ...
    const allMoneyFixed = '$' + String(allMoney).slice(0, 6) + '.' + String(allMoney).slice(6, 8)
